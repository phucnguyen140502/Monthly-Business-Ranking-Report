truncate table tmp_dvml;

INSERT INTO tmp_dvml
...
WITH regional_data AS (
    ...
    left join tmp_dt_nguon_von_da_phan_bo t on t.region_code = e.region_code
    left join tmp_cp_von_tt_1_da_phan_bo u on u.region_code = t.region_code
    left join tmp_cp_von_cctg_da_phan_bo w on w.region_code = u.region_code
    left join tmp_dt_fintech_da_phan_bo v on v.region_code = w.region_code
    left join tmp_dt_tieu_thuong_ca_nhan_da_phan_bo x on x.region_code = v.region_code
    left join tmp_cp_net_da_phan_bo y on y.region_code = x.region_code
    left join tmp_cp_thue_phi_da_phan_bo z on z.region_code = y.region_code
