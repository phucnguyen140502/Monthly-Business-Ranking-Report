truncate table hs_von_sale_manager;

INSERT INTO hs_von_sale_manager
...
WITH regional_data AS (
    ...
    left join tmp_cp_von_tt_2_da_phan_bo_sale_manager r on r.region_code = s.region_code and r.area_name = s.area_name and r.email = s.email
    left join tmp_cp_von_cctg_da_phan_bo_sale_manager u on r.region_code = u.region_code and r.area_name = u.area_name and r.email = u.email
),
calculate_hs_von as (
SELECT 
    month_key, 
...
FROM 
    regional_data)
select *, rank() over(order by hs_von desc)
from calculate_hs_von;


select*from hs_von_sale_manager;
